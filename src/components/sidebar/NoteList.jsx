  ...
  return (
    <div className="space-y-6">
        {notes.map((note) => (
            <div
                key={note.id}
                className="flex items-center gap-4 mr-auto group cursor-pointer"
            ...
            >
                ...
                <div>
                    <p className="text-grey-500 font-light mb-1" htmlFor="title">{note.createdAt}</p>
                    <p className="text-grey-100 transition group-hover:text-orange">{note.name}</p>
                </div>
            </div>
        ))}
    </div>
